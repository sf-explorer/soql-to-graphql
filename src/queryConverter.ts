 ...
 */
export function getArgs(parsedQuery: Query, input?: TInput): Record<string, unknown> {
    ...
    if (parsedQuery.orderBy) {
        const orderByArray = Array.isArray(parsedQuery.orderBy) ? parsedQuery.orderBy : [parsedQuery.orderBy];
        if (orderByArray.length > 0) {
            ...
            } else if ('functionName' in firstOrderBy && typeof firstOrderBy.functionName === 'string') {
                fieldName = firstOrderBy.functionName;
            } else if ('orderByField' in firstOrderBy && typeof firstOrderBy.orderByField === 'string') {
 ...
 */
export function getQuery(parsedQuery: QueryBase, input?: TInput): Record<string, unknown> {
