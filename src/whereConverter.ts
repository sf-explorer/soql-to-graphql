 ...
 */
export function getWhereField(cond: ValueCondition, input?: TInput): Record<string, unknown> {
    if (!cond || !cond.field) {
    ...
    let value: unknown;
    if (cond.literalType === LITERAL_TYPES.BOOLEAN && typeof cond.value === 'string') {
        value = cond.value === 'TRUE'
    } else if (cond.literalType === LITERAL_TYPES.INTEGER && typeof cond.value === 'string') {
        ...
        if (input && typeof value === 'string' && (value[0] === ':' || cond.literalType === LITERAL_TYPES.APEX_BIND_VARIABLE)) {
 ...
 */
export function getWhereCond(cond: unknown, input?: TInput): Record<string, unknown> {
    if (!cond) {
        throw new Error('Invalid condition: condition is required');
    }

    const condition = cond as { operator?: string; left?: unknown; right?: unknown };
